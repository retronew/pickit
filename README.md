...
- **Browser Rendering (optional):** the `browser` block in `wrangler.jsonc` binds it to the Worker; turn it on and pick your Workers plan under Settings → Data. It needs `compatibility_date` 2026-03-24 or later and doesn't run under local `wrangler dev` (capture falls back to plain text there). Remove the block if you don't want it.
