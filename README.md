...
- **Sharing**: public read-only links (`/s/:slug`) for a single bookmark, a live list of a whole category (with sub-categories) or tag, or a hand-picked collection (select bookmarks on the list page → Share); lists come with an RSS feed. Every link has a custom title, and the **Shares** page lists all links with visit stats (page views and RSS fetches per day, referrers, countries, recent visits; crawlers are skipped, the visit log is kept for 180 days)
