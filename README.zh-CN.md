...
- **定时维护**：检查失效链接（失效时自动查找 Wayback Machine 上最近的存档，详情页可「查看存档」）、清理过期审计日志；为旧收藏逐步补抓预览图（og:image），新收藏在「AI 识别」时直接获取。**设置 → 定时任务** 列出所有任务：执行时间（按你的时区显示，服务器按 UTC 执行）、上次执行结果和错误、下次执行时间、最近执行记录，以及「立即执行」按钮。每分钟的任务只在处理了内容或出错时记录，记录保留 30 天。
