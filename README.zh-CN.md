...
- **整理**：多级分类（如 `前端/React`），**分类**页可以重命名、移动（在分类树里选新的上级）、合并到其他分类或删除分类（其中的收藏和子分类上移一级；分享链接会跟着改名）；标签（重命名 / 删除）、访问次数统计、数据统计页
...
- **分享**：公开只读链接（`/s/:slug`），可以分享单条收藏，或一个或多个分类（含子分类）和标签下的收藏列表，也可以在收藏页多选后打包成合集分享；列表会自动更新并提供 RSS 订阅，访客可以平铺查看，也可以按分类或标签分组查看。分享标题可以自定义，还可以设置有效期（1 / 7 / 30 / 90 天）和访问密码（只保存加盐哈希；输入正确后可以查看页面和 RSS，改密码后旧的访问凭证随即失效），**分享**页集中管理所有链接并提供访问统计（每日页面访问与 RSS 拉取、独立访客、来源网站、地区，以及含浏览器、系统和设备的最近访问记录；忽略爬虫，同一访客 30 分钟内重复打开只计一次，不保存原始 IP，访问明细保留 180 天）
