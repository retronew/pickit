...
- **整理**：多级分类（如 `前端/React`），**分类**页可以重命名、移动（在分类树里选新的上级，或直接把分类拖到另一个分类上）、合并到其他分类或删除分类（其中的收藏和子分类上移一级；分享链接会跟着改名）；标签（重命名 / 删除）、「手动排序」（在同一分类内拖动收藏，排成自己的顺序）、访问次数统计、数据统计页
...
- **分享**：公开只读链接（`/s/:slug`），可以分享单条收藏，或一个或多个分类（含子分类）和标签下的收藏列表，也可以在收藏页多选后打包成合集分享（之后可在分享页拖动调整合集顺序）；列表会自动更新并提供 RSS 订阅，访客可以平铺查看，也可以按分类或标签分组查看。分享标题可以自定义，还可以设置有效期（1 / 7 / 30 / 90 天）和访问密码（只保存加盐哈希；输入正确后可以查看页面和 RSS，改密码后旧的访问凭证随即失效），**分享**页集中管理所有链接并提供访问统计（每日页面访问与 RSS 拉取、独立访客、来源网站、地区，以及含浏览器、系统和设备的最近访问记录；忽略爬虫，同一访客 30 分钟内重复打开只计一次，不保存原始 IP，访问明细保留 180 天）
...
- **API 访问**：Bearer API Token，方便脚本和第三方集成；另有 MCP 服务，AI 助手可以直接搜索和添加收藏；**Webhook**（设置 → 访问与分享）在新增、编辑、删除收藏或批量任务完成时 POST 带签名的 JSON（`X-PickIt-Signature: sha256=<请求体的 HMAC>`），可以发送测试并查看最近一次的发送结果
- **批量任务**：向量索引重建、AI 批量整理、AI 补充摘要和项目活跃度检查（被 GitHub 限流时自动暂停）按小批次分步执行（首页多选后也可以直接为选中的收藏生成摘要），支持暂停 / 继续、重试失败项，并能查看每一条的失败原因。设置页对应的标签打开时由页面推进，关闭后由每分钟一次的定时任务在后台继续
- **备份与恢复**：每天自动备份 JSON 到 R2（保留 30 天），也可以立即备份；在设置页的表格里下载、删除或恢复备份，恢复分「合并」（只补没有的网址）和「覆盖」（现有收藏移到回收站）两种方式，恢复前会自动备份当前数据，恢复错了也能撤回
- **定时维护**：检查失效链接（失效时自动查找 Wayback Machine 上最近的存档，详情页可「查看存档」）、清理过期审计日志；为旧收藏逐步补抓预览图（og:image），新收藏在「AI 识别」时直接获取。**设置 → 定时任务** 列出所有任务：执行时间（按你的时区显示，服务器按 UTC 执行）、上次执行结果和错误、下次执行时间、最近执行记录，以及「立即执行」按钮。每分钟的任务只在处理了内容或出错时记录；失败之后又正常运行过的任务会显示为运行正常，失败记录保留在历史里。遇到 D1 的临时错误（如「Network connection lost」）会先自动重试一次，仍然失败才记为失败。记录保留 30 天。
- **正文快照**（需要备份用的 R2 存储桶）：保存每个网页的可读正文（只取正文，去掉脚本和页面框架，最多 200 KB），存到 R2 的 `content/<id>.txt`，原网页改版或失效后依然能看。新收藏在后台抓取，旧收藏每分钟补几条；详情里显示抓取时间，可以打开阅读视图，也可以重新抓取。快照不会跟随原网页自动更新，重新抓取失败时保留原来的内容。抓取失败的会在 1、3、7 天后自动重试；修改收藏网址时会抓取新网页；在「设置 → 定时任务」的补抓任务上点「全部重新抓取」可把所有收藏重新排队。开启浏览器渲染后，旧的纯文本快照会逐步转成 Markdown，最多占用一半的用量上限，保证新收藏仍能渲染。需要登录或纯前端渲染的页面可能抓不到正文。
- **浏览器渲染正文（可选）**：`wrangler.jsonc` 里已带 `browser` 绑定（不需要 Account ID 或 API Token），在「设置 → 数据」开启 Cloudflare Browser Rendering 后，正文存成 Markdown，保留标题、列表、代码和图片，阅读视图按排版显示；纯前端渲染的页面也能抓到。不需要 JavaScript 就能读的页面只把正文 HTML 交给浏览器转换，每次耗时很短。浏览器时长按量计费，PickIt 会自己限制用量：可选 Workers 免费版（每天 10 分钟、每 10 秒 1 次请求，上限不能超过 10 分钟）或付费版（每月含 10 小时，默认上限 540 分钟，不产生额外费用）。达到上限或出错时自动改回普通抓取，设置卡片里能看到已用时长和重置时间。
- **项目活跃度**：GitHub 仓库和 npm 包会显示活跃程度：最后推送 / 更新时间、最新版本和发布时间、星标数或周下载量，以及活跃度分级（3 个月内有更新为活跃，1 年内为放缓，超过 1 年为不活跃，另有已归档 / 已弃用）。卡片上会标出不活跃和已归档的项目。新收藏立即检查，所有项目由定时任务约每周复查一次，详情里也可以手动刷新。GitHub 匿名请求每小时只有 60 次，建议在 **设置 → 访问与分享 → GitHub Token** 里填一个只读 token（也可以用 `GITHUB_TOKEN` 密钥），上限提高到每小时 5000 次。
...
- **API 路由测试**（`*.int.test.ts`）运行真实的 Worker（路由、登录校验和审计中间件），数据库是用真实迁移文件建出来的内存 SQLite（`apps/api/src/test/`，使用 Node 内置的 `node:sqlite`，需要 Node 22.13 以上）。覆盖收藏、回收站、批量操作、导入导出、搜索、标签、分类、分享（含有效期和密码）、设置、登录鉴权、审计日志、备份恢复、批量编辑（用模拟的 AI 模型）、Webhook（用模拟的 fetch）和 MCP。
