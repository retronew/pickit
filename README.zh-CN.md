...
- **正文快照**（需要备份用的 R2 存储桶）：保存每个网页的可读正文（只取正文，去掉脚本和页面框架，最多 200 KB），存到 R2 的 `content/<id>.txt`，原网页改版或失效后依然能看。新收藏在后台抓取，旧收藏每分钟补几条；详情里显示抓取时间，可以打开阅读视图，也可以重新抓取。快照不会跟随原网页自动更新，重新抓取失败时保留原来的内容。需要登录或纯前端渲染的页面可能抓不到正文。
