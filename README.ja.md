...
- **ブラウザで本文を取得（任意）**：「設定 → データ」で Cloudflare Browser Rendering をオンにする（Account ID と「Browser Rendering - Edit」権限の API トークンを入力）と、本文を Markdown で保存し、見出し・リスト・コード・画像をリーダー表示で整えて表示します。JavaScript で描画するページも取得できます。JavaScript なしで読めるページは本文の HTML だけをブラウザに渡すため、1 回の処理は短時間です。ブラウザ時間は従量制なので PickIt 側で使用量を制限します：Workers 無料プラン（1 日 10 分、10 秒に 1 リクエスト、上限は 10 分まで）か有料プラン（月 10 時間込み、既定の上限 540 分で追加料金なし）を選べます。上限に達したときやエラー時は通常の取得に戻ります。設定カードで使用時間とリセット日時を確認できます。
