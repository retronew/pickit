...
- **本文スナップショット**（バックアップ用の R2 バケットが必要）：各ページの読める本文（本文のみ、スクリプトやページの枠は除外、最大 200 KB）を R2 の `content/<id>.txt` に保存し、ページが変わったり消えたりしても読めます。新しいブックマークはバックグラウンドで、既存のものは 1 分ごとに数件ずつ取得します。詳細パネルで取得日時の確認、リーダー表示、再取得ができます。スナップショットは元ページに自動追従せず、再取得に失敗した場合は以前の本文を残します。ログインが必要なページや JavaScript だけで描画されるページは本文を取得できないことがあります。
