...
- **整理**：階層カテゴリ（例：`フロントエンド/React`）。**カテゴリ**ページで名前変更・移動（ツリーから新しい親を選択）・他のカテゴリへの統合・削除ができます（中のブックマークとサブカテゴリは 1 つ上に移動、共有リンクも名前変更に追従）。タグ（名前変更 / 削除）、アクセス回数、統計ページ
...
- **共有**：公開の読み取り専用リンク（`/s/:slug`）。単一のブックマーク、1 つまたは複数のカテゴリ（サブカテゴリ含む）とタグのリストのほか、一覧で複数選択したブックマークをコレクションとして共有できます。リストは自動更新され RSS も提供。閲覧者は一覧表示のほか、カテゴリ別・タグ別にまとめて表示できます。タイトルは自由に設定でき、有効期限（1 / 7 / 30 / 90 日）とパスワード（ソルト付きハッシュのみ保存。正しく入力するとページと RSS を閲覧でき、変更すると以前のアクセスは無効になります）も設定できます。**共有**ページですべてのリンクとアクセス統計（日別のページ閲覧と RSS 取得、ユニーク訪問者、参照元、国・地域、ブラウザ・OS・デバイス付きの最近のアクセス。クローラーは除外、同じ訪問者の 30 分以内の再訪問は 1 回として数え、生の IP は保存せず、明細は 180 日保存）を管理できます
