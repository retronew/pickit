...
- **定期メンテナンス**：リンク切れチェック（切れたリンクは Wayback Machine の最も近いアーカイブを探し、詳細画面で「アーカイブを見る」）、監査ログの整理、既存ブックマークのプレビュー画像（og:image）を少しずつ取得。新しいブックマークは「AI 解析」時に取得。**設定 → 定期タスク** ですべてのタスクの実行時刻（あなたのタイムゾーンで表示、サーバーは UTC で動作）、前回の結果とエラー、次回の実行時刻、最近の実行記録を確認でき、「今すぐ実行」もできます。毎分のタスクは何か処理したか失敗したときだけ記録し、記録は 30 日保存します。
