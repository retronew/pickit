...
- **共有**：公開の読み取り専用リンク（`/s/:slug`）。単一のブックマーク、カテゴリ（サブカテゴリ含む）やタグ全体のリストのほか、一覧で複数選択したブックマークをコレクションとして共有できます。リストは自動更新され RSS も提供。タイトルは自由に設定でき、**共有**ページですべてのリンクとアクセス統計（日別のページ閲覧と RSS 取得、参照元、国・地域、最近のアクセス。クローラーは除外、明細は 180 日保存）を管理できます
