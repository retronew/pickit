...
**眠らせないブックマーク。** Cloudflare Workers + D1 だけで動く、セルフホスト型のシングルユーザー向けブックマークマネージャーです。リンクを保存し、カテゴリとタグで整理し、全文検索やセマンティック検索で探せます。AI に整理を手伝わせることもできます。

## 機能

- **ブックマーク管理**：追加、編集、ピン留め、ゴミ箱への移動と復元、重複の検出と統合
- **一括編集**：選択した項目のピン留め、カテゴリ移動、削除、タグの追加 / 削除。AI にカテゴリとタグを提案させ、表で確認して選んだものだけ適用することも可能
- **整理**：階層カテゴリ（例：`フロントエンド/React`）。**カテゴリ**ページで名前変更・移動（ツリーから新しい親を選択、またはカテゴリを別のカテゴリへドラッグ）・他のカテゴリへの統合・削除ができます（中のブックマークとサブカテゴリは 1 つ上に移動、共有リンクも名前変更に追従）。タグ（名前変更 / 削除）、「手動で並べ替え」（同じカテゴリ内でブックマークをドラッグして好きな順に）、アクセス回数、統計ページ
- **検索**：SQLite FTS5 による全文検索と、embedding によるセマンティック検索。類似度計算は 512 次元の圧縮ベクトルで候補を絞り込み、完全なベクトルで再ランキングするため、4096 次元のモデルでも無料プランの CPU 制限（10 ms）に収まります。検索語・カテゴリ・タグ・並び順は「保存した検索」としてサーバーに保存でき、端末間で同期されます
...
- **共有**：公開の読み取り専用リンク（`/s/:slug`）。単一のブックマーク、1 つまたは複数のカテゴリ（サブカテゴリ含む）とタグのリストのほか、一覧で複数選択したブックマークをコレクションとして共有できます（共有ページでドラッグして順序を変更可能）。リストは自動更新され RSS も提供。閲覧者は一覧表示のほか、カテゴリ別・タグ別にまとめて表示できます。タイトルは自由に設定でき、有効期限（1 / 7 / 30 / 90 日）とパスワード（ソルト付きハッシュのみ保存。正しく入力するとページと RSS を閲覧でき、変更すると以前のアクセスは無効になります）も設定できます。**共有**ページですべてのリンクとアクセス統計（日別のページ閲覧と RSS 取得、ユニーク訪問者、参照元、国・地域、ブラウザ・OS・デバイス付きの最近のアクセス。クローラーは除外、同じ訪問者の 30 分以内の再訪問は 1 回として数え、生の IP は保存せず、明細は 180 日保存）を管理できます
...
- **キーボードショートカット**：`/` 検索、`N` 追加、`J`/`K` カード間を移動、`Enter` 詳細、`O` リンクを開く、`E` 編集、`P` ピン留め。`?` で一覧を表示
...
- **API アクセス**：スクリプトや外部連携用の Bearer API トークン。AI アシスタントが検索・追加できる MCP サーバーも提供。**Webhook**（設定 → アクセスと共有）はブックマークの追加・編集・削除や一括処理の完了時に署名付き JSON（`X-PickIt-Signature: sha256=<本文の HMAC>`）を POST し、テスト送信と最終送信結果の確認もできます
- **バッチジョブ**：埋め込みの再生成、AI 一括整理、AI 要約の一括生成、プロジェクトの活動状況の確認（GitHub のレート制限時は自動で一時停止）は（ホームで選択したブックマークの要約を直接生成することも可能）小さなステップに分けて実行され、一時停止 / 再開、失敗項目の再試行、項目ごとのエラー詳細に対応。設定ページの該当タブを開いている間はページが処理を進め、閉じた後は毎分の cron がバックグラウンドで続行します
...
- **定期メンテナンス**：リンク切れチェック（切れたリンクは Wayback Machine の最も近いアーカイブを探し、詳細画面で「アーカイブを見る」）、監査ログの整理、既存ブックマークのプレビュー画像（og:image）を少しずつ取得。新しいブックマークは「AI 解析」時に取得。**設定 → 定期タスク** ですべてのタスクの実行時刻（あなたのタイムゾーンで表示、サーバーは UTC で動作）、前回の結果とエラー、次回の実行時刻、最近の実行記録を確認でき、「今すぐ実行」もできます。毎分のタスクは何か処理したか失敗したときだけ記録します。失敗後に問題なく動いたタスクは正常と表示し、失敗は履歴に残します。D1 の一時的なエラー（「Network connection lost」など）は 1 回自動で再試行し、それでも失敗した場合のみ失敗として記録します。記録は 30 日保存します。
- **本文スナップショット**（バックアップ用の R2 バケットが必要）：各ページの読める本文（本文のみ、スクリプトやページの枠は除外、最大 200 KB）を R2 の `content/<id>.txt` に保存し、ページが変わったり消えたりしても読めます。新しいブックマークはバックグラウンドで、既存のものは 1 分ごとに数件ずつ取得します。詳細パネルで取得日時の確認、リーダー表示、再取得ができます。スナップショットは元ページに自動追従せず、再取得に失敗した場合は以前の本文を残します。取得に失敗したものは 1・3・7 日後に再試行し、ブックマークの URL を変更すると新しいページを取得します。「設定 → 定期タスク」の取得タスクにある「すべて再取得」で全ブックマークを再びキューに入れられます。ブラウザレンダリングがオンなら、古いテキストのスナップショットを少しずつ Markdown に変換します（上限の半分まで。新しいブックマークの分は残します）。ログインが必要なページや JavaScript だけで描画されるページは本文を取得できないことがあります。
- **ブラウザで本文を取得（任意）**：`wrangler.jsonc` に `browser` バインディングが含まれており（Account ID や API トークンは不要）、「設定 → データ」で Cloudflare Browser Rendering をオンにすると、本文を Markdown で保存し、見出し・リスト・コード・画像をリーダー表示で整えて表示します。JavaScript で描画するページも取得できます。JavaScript なしで読めるページは本文の HTML だけをブラウザに渡すため、1 回の処理は短時間です。ブラウザ時間は従量制なので PickIt 側で使用量を制限します：Workers 無料プラン（1 日 10 分、10 秒に 1 リクエスト、上限は 10 分まで）か有料プラン（月 10 時間込み、既定の上限 540 分で追加料金なし）を選べます。上限に達したときやエラー時は通常の取得に戻ります。設定カードで使用時間とリセット日時を確認できます。
- **プロジェクトの活動状況**：GitHub リポジトリと npm パッケージの活動状況（最終プッシュ / 更新日時、最新バージョンと公開日、スター数または週間ダウンロード数、活発（3 か月以内）・鈍化（1 年以内）・停滞（1 年超）・アーカイブ / 非推奨の区分）を表示します。カードには停滞・アーカイブ済みのプロジェクトを表示。新しいブックマークはすぐに、既存のものは定期処理でおよそ週 1 回確認し、詳細パネルから今すぐ更新もできます。GitHub の匿名リクエストは 1 時間 60 回までなので、**設定 → アクセスと共有 → GitHub Token** に読み取り専用トークン（または `GITHUB_TOKEN` シークレット）を設定すると 1 時間 5,000 回になります。
...
- **API ルートテスト**（`*.int.test.ts`）は本物の Worker（ルート、認証・監査ミドルウェア）を、実際のマイグレーションで作ったインメモリ SQLite 上で実行します（`apps/api/src/test/`、Node 組み込みの `node:sqlite` を使うため Node 22.13 以上が必要）。ブックマーク、ゴミ箱、一括操作、インポート / エクスポート、検索、タグ、カテゴリ、共有（有効期限・パスワード含む）、設定、認証、監査ログ、バックアップと復元、一括編集（AI モデルはモック）、Webhook（fetch はモック）、MCP をカバーします。
...
- **Browser Rendering（任意）**：`wrangler.jsonc` の `browser` ブロックで Worker にバインドされます。デプロイ後「設定 → データ」でオンにし、Workers プランを選びます。`compatibility_date` は 2026-03-24 以降が必要で、ローカルの `wrangler dev` では動かず、取得は通常のテキストに戻ります。不要ならこのブロックを削除してください。
